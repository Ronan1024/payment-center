...
import com.baosight.payment.accounting.mapper.ChannelBillFileMapper;
...
public class TongLianPayReconciliationManagerImpl implements TongLianPayReconciliationManager {
    ...
    public Boolean handlerPayAgencyBill(MchInterfaceConfigVO mchInterfaceConfig, String date) {
        ...
        // 创建对账批次 渠道-商户-时间-交易类型
        // 消费对账
        String checkBatchConsumption = mchInterfaceConfig.getInterfaceCode() + "-" + mchInterfaceConfig.getMchId() + "-" + date + "-" + TradeType.CONSUMPTION.getCode();
        // 退款对账
        String checkBatchRefund = mchInterfaceConfig.getInterfaceCode() + "-" + mchInterfaceConfig.getMchId() + "-" + date + "-" + TradeType.REFUND.getCode();
    ...
    private void parse(ChannelBill channelBill, String tongLianBillLine, String date, MchInterfaceConfigVO mchInterfaceConfig) {
        ...
        //订单类型
        TradeType apply = orderTypeFunction.apply(split[1]);
...
//        channelBill.setUpstreamChannelOrderId(String.valueOf(split[15]));
    ...
    private Function<String, TradeType> orderTypeFunction = e -> switch (e) {
        // TODO 通联未处理 订单类型"2089", "2297", "2080", "2084", "2094"
        case "2085" -> TradeType.CONSUMPTION;
        case "2294" -> TradeType.REFUND;
        case "2290" -> TradeType.WITHDRAWAL;
        case "4020" -> TradeType.SETTLEMENT;
        case "2298" -> TradeType.ERROR;
        default -> null;
    };
